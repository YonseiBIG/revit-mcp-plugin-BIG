...
namespace revit_mcp_plugin.UI
{
    ...
    public partial class CommandSetSettingsPage : Page
    {
                ...
                foreach (var directory in commandSetDirectories)
                {
                    ...
                    if (File.Exists(commandJsonPath))
                    {
                        ...
                        if (commandSetData != null)
                        {
                            ...
                            {
                                // Create a command configuration, but determine supported versions by inspecting folders
                                ...
                                foreach (var version in versionDirectories)
                                {
                                    ...
                                    {
                                        // If a relative path is specified, look for it inside the version subfolder
                                        ...
                                        if (File.Exists(versionDllPath))
                                        {
                                            ...
                                            if (dllBasePath == null)
                                            {
                                                // Extract relative path, used to create the template
                                    ...
                                    {
                                        // If no path is specified, look for any DLL inside the version subfolder
                                ...
                                if (supportedCommandVersions.Count > 0 && dllBasePath != null)
                                {
                                    ...
                                    var commandConfig = new CommandConfig
                                    {
                                        ...
                                        // Use the path that contains the version placeholder
                            ...
                            // If there are available commands, add to the command set list
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                string registryFilePath = PathManager.GetCommandRegistryFilePath();
                // Read the existing registry to preserve full command information
                ...
                foreach (var commandSet in commandSets)
                {
                    ...
                    foreach (var command in commandSet.Commands)
                    {
                        ...
                        if (command.Enabled)
                        {
                            ...
                            {
                                // If it exists, preserve the original info and only update the enabled status and path template
                            ...
                            {
                                // If this is a new command, create a new configuration
